...
export class OrdersService {
  ...
  async create(userId: string, dto: CreateOrderDto) {
    ...
    const area = await this.deliveryService.getActiveAreaById(dto.deliveryAreaId);
    if (!area) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const score = cartItems.reduce(
        (sum, item) =>
          sum.plus(
            new Prisma.Decimal(item.product.freeDeliveryValue).times(item.quantity),
          ),
  ...
  private assertPurchasable(item: CartItemWithRelations) {
    ...
    if (product.availability === ProductAvailability.UNAVAILABLE) {
      throw new ValidationException(`Product '${product.name}' cannot be purchased`);
    }
  ...
  buildOrderItemSnapshot(item: CartItemWithRelations): Prisma.OrderItemCreateWithoutOrderInput {
